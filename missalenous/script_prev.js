...
//                fetchForecastData(regionCfg.var_label, 'forecast_prev_fitted.json')
            ...
            /**
             * Merges a dataset into the `mergedData` object, handling both past and current data.
             * @param {Array} data - The dataset to merge.
             * @param {boolean} isPastData - Whether the dataset represents past data.
             */
...
async function updateTotalLineChart(config, finalSeries, loadSeries, forecastStartTime) {
  const lineChart = stackedChartState[`lineChartInstance${config.chartId}`];
  const isDarkMode = config.isDarkMode;
  if (!lineChart) return;

  // Summation of the energy generation data
  const totalSeries = computeTotalSeries(finalSeries);

  // Ensure `loadSeries` has correct x-value formatting
  const formattedLoadSeries = loadSeries.map(series => ({
    ...series,
    data: series.data.map(({ x, y }) => [new Date(x).getTime(), y]) // Convert to timestamps
  }));

  // Ensure `totalSeries` has correct x-value formatting
  const formattedTotalSeries = totalSeries.map(series => ({
    ...series,
    data: series.data.map(([x, y]) => [new Date(x).getTime(), y]) // Convert to timestamps
  }));

  // Merge both datasets
  const allSeries = [...formattedTotalSeries, ...formattedLoadSeries];

  // Optional: Forecast start marker
const annotations = forecastStartTime
  ? [
      {
        x: forecastStartTime,
        borderColor: '#555', // Gray line color
        strokeDashArray: 5,
        label: {
          borderColor: '#333', // Dark gray border
          position: 'bottom',
          offsetX: -5, // Move text to the right
          text: 'Actual',
          style: {
            background: '#333', // Dark gray background
            color: '#FFF', // White text
            fontSize: '12px',
            rotate: -180, // Rotates text by 90 degrees counterclockwise
          },
        },
      },
      {
        ...
        label: {
          borderColor: '#333', // Dark gray border
          position: 'bottom',
          offsetX: 25, // Move text to the right
          text: 'Forecast',
          style: {
            background: '#333', // Dark gray background
            color: '#FFF', // White text
            fontSize: '12px',
            rotate: -90, // Ensures vertical orientation, pointing text outward
          },
        },
      },
    ]
  ...
  lineChart.updateSeries(allSeries);
  ...
  lineChart.updateOptions({
    ...
    legend: {
      ...
      formatter: function(seriesName, opts) {
        return opts.w.globals.seriesNames.indexOf(seriesName) < 2 ? seriesName : null;
      }
    },
  });
}
...
function updateStackedChart(config, finalSeries, forecastStartTime) {
    ...
    // Add vertical annotation at forecast start time if available
const annotations = forecastStartTime
  ? [
      {
        x: forecastStartTime,
...
function setupStackedChartEvents({
  ...
  // Event listener for toggling the markdown description section
  document
    .getElementById(descriptionToggleId)
    ?.addEventListener('click', async function() {
      const content = document.getElementById(descriptionContainerId);
  ...
  // Event listener for when the user toggles the "details" (expands/collapses the chart section)
  document
    .querySelector(detailsSelector)
    .addEventListener('toggle', async function(e) {
  ...
  // Function to destroy & recreate the charts when "RESET" is clicked
  ...
  // Function to update both stacked area and total generation line charts
  window[`updateStackedChart${stackedChartNum}`] = async function() {
    ...
    const { finalSeries, forecastStartTime } = await processStackedChartData(config);
    let combinedLoadSeries = [];

    for (const regionCfg of config.regionConfigs) {
      const checkBox = document.getElementById(regionCfg.checkboxId);
      if (!checkBox || !checkBox.checked) continue;

      const prevActualData = await getCachedData(
        regionCfg.load_var_label, 'forecast_prev_actual.json', config.errorElementId
      );
      const currFittedData = await getCachedData(
        regionCfg.load_var_label, 'forecast_curr_fitted.json', config.errorElementId
      );

      if (prevActualData || currFittedData) {
        const combinedData = [];
        const pastDataRatio = document.getElementById(config.pastDataSliderId)?.value / 100 || 1;

        if (prevActualData) {
          combinedData.push(...prevActualData.slice(-Math.floor(prevActualData.length * pastDataRatio)));
        }
        if (currFittedData) {
          combinedData.push(...currFittedData);
        }

        combinedLoadSeries.push({
          name: `${regionCfg.alias} Load`,
          data: combinedData,
          color: '#008080'
        });
      }
    ...
    updateStackedChart(config, finalSeries, forecastStartTime);
    updateTotalLineChart(config, finalSeries, combinedLoadSeries, forecastStartTime);
  };
}
