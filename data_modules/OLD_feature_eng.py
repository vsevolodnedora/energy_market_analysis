...
from data_collection_modules.locations import all_locations
...
def compute_gust_factor(wind_speed:pd.Series):
    ...
    gf = gf.replace([np.inf, -np.inf], np.nan)
    return gf
...
class WeatherLoadFE:
    ...
    def _preprocess_location( self, df: pd.DataFrame, location_suffix: str ) -> pd.DataFrame:
        ...
        # ------------------------------------------------------------------------
        # 3.1 Heating and Cooling Degree Hours
        if self.config.get("compute_heating_degree_hours", False):
            hdh_threshold = self.config.get("hdh_threshold", 18)
            loc_df[f"HDH{location_suffix}"] = np.clip(hdh_threshold - loc_df[temp_col], 0, None)

        if self.config.get("compute_cooling_degree_hours", False):
        ...
        if self.config.get("compute_dew_point_spread", False):
            ...
            loc_df[f"dew_point_spread{location_suffix}"] = loc_df[temp_col] - loc_df["dew_point_temperature" + location_suffix]

        # 3.3 Temperature Gradient (one-hour difference)
        if self.config.get("compute_temp_gradient", False):
        ...
        # ------------------------------------------------------------------------
        if self.config.get("compute_wind_chill", False):
            wind_speed_ms = loc_df[wind_speed_10m_col] / 3.6  # convert km/h -> m/s
            T_C = loc_df[temp_col]
            loc_df[f"wind_chill{location_suffix}"] = (
                    13.12 + 0.6215 * T_C
                    - 11.37 * (wind_speed_ms ** 0.16)
                    + 0.3965 * T_C * (wind_speed_ms ** 0.16)
            )

        if self.config.get("compute_humidex", False):
            T_C = loc_df[temp_col]
            RH = loc_df[humid_col] / 100.0
            e_hpa = 6.112 * np.exp((17.67 * T_C) / (T_C + 243.5)) * RH
            e_pa = e_hpa * 100.0
            dewpoint_term = 0.5555 * (e_pa / 100.0 - 10.0)
            loc_df[f"humidex{location_suffix}"] = T_C + dewpoint_term

        # ------------------------------------------------------------------------
        # 5. Pressure-based features (pressure trend, air density if needed)
        # ------------------------------------------------------------------------
        # 5.1 Pressure Trend
        if self.config.get("compute_pressure_trend", False):
        ...
        if self.config.get("compute_air_density", False):
            R_d = 287.05  # J/(kg*K)
            loc_df[f"air_density{location_suffix}"] = (
                    loc_df[press_col] * 100.0 / (R_d * (loc_df[temp_col] + 273.15))
            )

        # ------------------------------------------------------------------------
        # 6. Precipitation features (binary indicator, existing rolling sums, etc.)
        # ------------------------------------------------------------------------
        # 6.1 Binary Rain Indicator
        if self.config.get("compute_rain_indicator", False):
            loc_df[f"rain_indicator{location_suffix}"] = (loc_df[precip_col] > 0).astype(int)

        # 6.2 Lags or rolling sums of precipitation remain as in your original code
        if "precip_lags" in self.config and self.config["precip_lags"]:
            for lag in self.config["precip_lags"]:
                loc_df[f"precip_lag_{lag}{location_suffix}"] = loc_df[precip_col].shift(lag)
        ...
        # ------------------------------------------------------------------------
        # 7.1 Wind Speed Gradient
        if self.config.get("compute_wind_speed_gradient", False):
        ...
        if self.config.get("compute_wind_components", False) and wind_dir_10m_col in loc_df.columns:
        ...
        if self.config.get("compute_wind_power_density", False):
            density_col = f"air_density{location_suffix}"
            if density_col in loc_df.columns and wind_speed_100m_col in loc_df.columns:
                wind_speed_ms_100m = loc_df[wind_speed_100m_col] / 3.6
                loc_df[f"wind_power_density_100m{location_suffix}"] = (
                        0.5 * loc_df[density_col] * (wind_speed_ms_100m ** 3)
                )
        ...
        if self.config.get("compute_cloud_cover_fraction", False) and cloud_col in loc_df.columns:
            loc_df[f"cloud_cover_fraction{location_suffix}"] = loc_df[cloud_col] / 100.0

        # 8.2 Effective Solar Radiation: (1 - cloud_cover) * shortwave_radiation
        if (
                self.config.get("compute_effective_solar", False)
                and shortwave_col in loc_df.columns
                and cloud_col in loc_df.columns
        ):
            # If fraction not yet computed, do it here
            if f"cloud_cover_fraction{location_suffix}" not in loc_df.columns:
                cloud_fraction = loc_df[cloud_col] / 100.0
            else:
                cloud_fraction = loc_df[f"cloud_cover_fraction{location_suffix}"]
            loc_df[f"effective_solar{location_suffix}"] = (
                    (1 - cloud_fraction) * loc_df[shortwave_col]
            )

        # 8.3 Drop raw radiation features if requested (from original code)
        if self.config.get("drop_radiation_features", False):
            rad_cols = [
                shortwave_col, direct_col, diffuse_col,
                dni_col, global_tilted_col, terrestrial_col
            ]
            loc_df.drop(columns=[c for c in rad_cols if c in loc_df.columns],
                        inplace=True, errors="ignore")

        # ------------------------------------------------------------------------
        # 9. Lags (temp, cloud, etc.) and rolling features (kept from original)
        # ------------------------------------------------------------------------
        if "temp_lags" in self.config and self.config["temp_lags"]:
            for lag in self.config["temp_lags"]:
        ...
        if "rolling_temp" in self.config:
            for window in self.config["rolling_temp"]:
        ...
        # ------------------------------------------------------------------------
        if self.config.get("drop_basic_meteo_features", False):
        ...
        if self.config.get("drop_wind_meteo_features", False):
            loc_df.drop(columns=[c for c in wind_feat if c in loc_df.columns],
                        inplace=True, errors="ignore")

        if self.config.get("drop_rad_meteo_features", False):
    ...
    def selector_for_optuna(trial: optuna.Trial, fixed: dict) -> dict:
        ...
        #    Example approach for controlling sets of lags
        # ----------------------------------------------------------------
        temp_lags_choice = trial.suggest_categorical(
            "temp_lags_option", ["none", "small", "medium"]
        )
        if temp_lags_choice == "none":
            temp_lags = []
        elif temp_lags_choice == "small":
            temp_lags = [1, 3]
        else:  # "medium"
            temp_lags = [1, 3, 6]
        ...
        rolling_temp_choice = trial.suggest_categorical(
            "rolling_temp_option", ["none", "short", "long"]
        )
        if rolling_temp_choice == "none":
            rolling_temp = []
        elif rolling_temp_choice == "short":
            rolling_temp = [3, 6]  # 3-hour, 6-hour rolling avg
        else:  # "long"
            rolling_temp = [6, 12, 24]

        # ----------------------------------------------------------------
        # 5. Dropping raw or radiation features
        # ----------------------------------------------------------------
        drop_radiation_features = trial.suggest_categorical(
            "drop_radiation_features", [True, False]
        )
        ...
        #    We include the new methods from load forecasting: "population",
        #    "energy", "distance_population", "distance_energy", plus "mean", etc.
        # ----------------------------------------------------------------
        if "spatial_agg_method" not in fixed:
            spatial_agg_method = trial.suggest_categorical(
                "spatial_agg_method",
        ...
        config = {
            ...
            # Dropping raw features
            "drop_radiation_features": drop_radiation_features,
...
def physics_informed_feature_engineering(df_hist_:pd.DataFrame, df_forecast_:pd.DataFrame, config:dict, verbose:bool):
    ...
    if config['feature_engineer'] in ['WeatherWindPowerFE','WeatherSolarPowerFE','WeatherLoadFE']:
        ...
        df_tmp = validate_dataframe(df_tmp, level='info')
