...
class WeatherWindPowerFE:
    ...
    def _apply_spatial_aggregation_OLD(self, df: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    def _apply_spatial_aggregation(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.verbose: print(f"Aggregating features across multiple location: "
                               f"Locations: {len(list(features_per_wind_farm.keys()))}")
        ...
        for loc in self.locations:
            ...
            for col_name in features_per_wind_farm[suffix]:
                ...
                if base_feat not in base_feature_map:
                    base_feature_map[base_feat] = []
                base_feature_map[base_feat].append((suffix, col_name))
        if self.verbose: print(f"Stripping suffixes. "
                               f"N_base_features = {len(list(base_feature_map.keys()))}")
...
def physics_informed_feature_engineering(df_hist_:pd.DataFrame, df_forecast_:pd.DataFrame, config:dict, verbose:bool):
    ...
    if config['feature_engineer'] in ['WeatherWindPowerFE','WeatherSolarPowerFE','WeatherLoadFE']:
        ...
        df_tmp = validate_dataframe(df_tmp, log_func=logger.info)
